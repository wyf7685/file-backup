...
def _init_7zip() -> tuple[Path, Path]:
...
def _execute_7z(args: List[str]):
    ...
    return "Everything is Ok" in output.decode("gbk") and p.returncode == 0, err.decode(
        "gbk"
    )
...
def _pack_7zip_multipart(
    ...
    if success:
        return [
            ...
            if p.is_file() and p.name.startswith(archive.name) and p.name.split(".")[-1].isdigit()
        ]
    raise RuntimeError(f"压缩文件错误: {err}")
